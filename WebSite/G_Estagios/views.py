...
def registo(request):
    ...
    if V_Email:
        ...
        Password=request.POST.get('Password')
        Nome = request.POST.get('Nome')
        is_password_secure, error_message = verificar_palavra_passe(Password)
        V_e = verify_email(Email)
        if is_password_secure:
            if V_e == "Invalido":
                ...
                newuser = User.objects.create_user(username=Email,first_name=Nome,password=is_password_secure)
                newuser.save()
                messages.info(request,"Registo bem sucedido")
                return redirect('register')
            elif V_e == 'Professor':
